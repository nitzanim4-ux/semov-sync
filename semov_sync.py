...
SEMOV_BASE_URL       = "https://api.smov.co.il/v1"      # עדכן אם שונה
...
def get_contacts(list_id: str) -> list[dict]:
    ...
    while True:
        resp = requests.get(
            f"{SEMOV_BASE_URL}/contacts",
            headers=headers,
            params={"list_id": list_id, "page": page, "per_page": 100},
            timeout=30,
        )
...
def build_email_html(missing: list[dict]) -> str:
    ...
    for c in missing:
        ...
        phone   = (
            c.get("mobile") or c.get("phone_mobile")
    ...
    <html><body style="font-family:Arial,sans-serif;color:#333;max-width:700px;margin:auto;">
      <h2 style="color:#1a73e8;">דוח חסרים שבועי – {today}</h2>
...
def build_email_text(missing: list[dict]) -> str:
    today = datetime.now().strftime("%d/%m/%Y")
    lines = [f"דוח חסרים שבועי – {today}", f"סה\"כ: {len(missing)} חסרים\n"]
    lines += [
        f"{c.get('first_name','')} {c.get('last_name','')} | {c.get('company','')} | "
        f"{c.get('mobile') or c.get('phone_mobile') or c.get('cell') or c.get('phone','')}"
        for c in missing
    ]
    return "\n".join(lines)
...
def send_email(missing: list[dict]):
    today = datetime.now().strftime("%d/%m/%Y")
    subject = f"דוח חסרים שבועי – {today} ({len(missing)} אנשי קשר)"
